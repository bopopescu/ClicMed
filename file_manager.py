...
def upload_file(infos, root_frame, username, group):
    ...
    label = settings.tk.Label(popup, text="Upload sucessfull !", font=("Arial", 11), fg='white', bg='#3c3f41')
...
def download_file(file_selected, username):
    ...
    popup.grab_set()
    popup.tk.call('wm', 'iconphoto', popup._w, ico_popup)
    label = settings.tk.Label(popup, text="Upload Finished !", font=("Arial", 11), fg='white', bg='#3c3f41')
    label.pack(side="top", fill="x", pady=10)
    okay_btn = settings.tk.Button(popup, text="Okay", command=popup.destroy)
    okay_btn.pack()
    popup.mainloop()
